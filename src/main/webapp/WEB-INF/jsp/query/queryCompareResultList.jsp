    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form id="consignationForm" action="<%=path %>/queryCompareResult/querySameCompareResultList"
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="select">
                                    <select class="form-control">
                                        ...
                                        <option value="" selected>待复核</option>
                                    </select>
                                </div>
                            </div>
                        </div>--%>
                        ...
                        <div class="col-md-4 seachInputBox">
                            <div class="form-group">
                                ...
                                <c:choose>
                                    <c:when test="${matchResult.entity.sameCount!=null&&matchResult.entity.sameCount!=''}">
                                        <input type="text" class="form-control sameCount" name="entity.sameCount" value="${matchResult.entity.sameCount}" placeholder="请输入匹配数">
                                    </c:when>
                                    <c:otherwise>
                                        <input type="text" class="form-control sameCount"  name="entity.sameCount" value="13" placeholder="请输入匹配数">
                                    </c:otherwise>
                                </c:choose>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4 seachInputBox">
                            <div class="form-group">
                                <label>比对结果类别</label>
                                <div class="select">
                                    <select id="comparisonCategory" name="comparisonCategory"
                                            value="${matchResult.comparisonCategory}" class="form-control">
                                        <c:forEach items="${comparisonCategoryList}" var="list" varStatus="cs">
                                            <option value="${list.dictCode}"
                                                    <c:if test="${list.dictCode eq matchResult.comparisonCategory}">selected</c:if>>${list.dictName}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                            </div>
    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading yellow">
                <div>串并案列表</div>
                <button class="btn btn-blue exportListExcel">批量导出确认比中结果Excel</button>
                <div class="pull-right">
                    <label class="custom-control checkbox-inline beforeCheck">
                        <input class="custom-control-input" type="checkbox" name="checkAll">
                        <span class="" style="font-weight: 600 ;margin-left: 6px;">全选</span>
                    </label>
                    <button class="btn btn-danger relieveGroupRelevanceAll" style="margin-right: 5px;">
                        解除关联
                    </button>
                    <%--<button class="btn-blue btn pull-right btn-pink relieveGroupRelevance" type="button">
                        解除关联
                    </button>--%>
                    <button type="button" class="btn-light-blue btn pull-right confirmCompareGroup">
                        确认比中
                    </button>
                </div>
            </div>
                    ...
                    <c:forEach items="${matchResultList}" var="matchResult" varStatus="s">
                        <div class="col-md-12 TableBox">
                            <table class="table table-hover table-bordered bigTable">
                                ...
                                <tr>
                                    <th colspan="1">
                                        <label class="custom-control checkbox-inline">
                                            <input type="checkbox" class="custom-control-input" name="checkboxCase">
                                            <span class="custom-control-label" style="font-weight: 600" id="stage${s.index+1}">
                                                <script>toZhDigit(${s.index+1})</script>
                                            </span>
                                        </label>
                                    </th>
                                    <%--<th colspan="1">第${s.index+1}组</th>--%>
                                    <th colspan="1">
                                        <c:set var="isDone" value="0" scope="page"></c:set>
                                        <c:forEach items="${matchResult.value}" var="result" varStatus="c">
                                            <c:if test="${ isDone == '0' }">
                                                <a href="<%=path%>/queryCompareResult/chuanbindan?groupId=${result.entity.groupId}">
                                                    <button type="button" class="btn btn-red btn-block">案件串并文件生成</button>
                                                </a>
                                            </c:if>
                                    ...
                                    <th colspan="7">
                                        <div class="row">
                                            <div class="col-md-5 noPadding">
                                                <div class="form-group">
                                                    <label class="col-md-2 noPadding"
                                                           style="height: 34px;line-height: 34px">案件比对</label>
                                                    <div class="col-md-10 noPadding">
                                                        <div class="select">
                                                            <select class="form-control" id="caseId" name="caseId">
                                                                <option value="">请选择案件比对</option>
                                                                <c:forEach items="${resultList}" var="resultList" varStatus="c">
                                                                    <c:if test="${matchResult.key eq resultList.key}">
                                                                        <c:forEach items="${resultList.value}" var="result" varStatus="c">
                                                                            <option  value="${result.caseId}">${result.caseName}</option>
                                                                        </c:forEach>
                                                                    </c:if>
                                                                </c:forEach>
                                                            </select>
                                            ...
                                            <div class="col-md-7">
                                                <input type="hidden" name="generateGroupId" value="${matchResult.key}">
                                                <button class="btn-yellow-border btn " name="generateIdentifyBook">生成串并案鉴定书</button>
                                                <%--<button class="btn-blue btn">
                                                    <a href="<%=path%>/queryCompareResult/chuanbindanJD?groupId=${matchResult.key}">
                                                        <i class="fa fa-download" aria-hidden="true" style="color: #fff"></i>
                                                    </a>
                                                </button>--%>
                                                <a>
                                                <button class="btn-blue-border btn pull-right" name="queryImgUrl"
                                                        style="margin-right: 10px">打印资格证书
                                                </button>
                                                </a>
                                            </div>
                                        </div>
                                ...
                                <tbody>
                                <c:forEach items="${matchResult.value}" var="result" varStatus="c">
                                <%--<c:forEach items="${result2List}" var="result1" varStatus="c">--%>
                                <%--<c:forEach items="${result1.value}" var="result" varStatus="l">--%>
                                    <tr>
                                        <td>${c.index+1}</td>
                                        <td>${result.caseName}</td>
                                        <td>未破获1</td>
                                        <td class="clickRed">${result.sampleNo}</td>
                                        <td>${result.sampleName}</td>
                                        <td>${result.instoredType}</td>
                                        <td>${result.entity.matchType}</td>
                                        <td>${result.delegateOrgName}</td>
                                        <td>
                                            <input type="hidden" name="groupId"
                                                   value="${result.entity.groupId}">
                                            <input type="hidden" name="sampleNo"
                                                   value="${result.sampleNo}">
                                            <input type="hidden" name="id"
                                                   value="${result.entity.id}">
                                            <button type="button" name="queryDetail"
                                                    class="btn-icon btn-icon-yellow btn-icon-chakan-yellow">查看详情
                                            </button>
                                        </td>
                                    </tr>
                                </c:forEach>
...
<script>
    $(function () {
        ...
        $("button[name='generateIdentifyBook']").on("click",function(){
            ...
            var groupId = $("input[name='generateGroupId']", $(this).parent()).val();
            ...
            $.ajax({
                url:"<%=path%>/center/generateChuanBingIdentifyBook?caseId=" + caseId + "&groupId=" + groupId,
                type:"get",
                dataType:"json",
        ...
        kkpager.generPageHtml({
            ...
            //链接前部
            hrefFormer: '<%=path%>/queryCompareResult/querySameCompareResultList',
            //链接尾部
            //hrefLatter: '.html',
        ...
        $("button[name='queryDetail']").on("click", function () {
            ...
            location.href = "<%=path%>/queryCompareResult/queryCompareResultDetail?groupId=" + groupId + "&groupNo=" + groupNo+ "&sampleNo=" + sampleNo;
        })
...
//                            alert("选中的案件不是当前分局的案件")
        ...
        $("button[name='queryImgUrl']").click(function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    console.log(data.orgImg)
                    console.log(data.amPersonalAptitudeOne.aptitudePicture)
                    console.log(data.amPersonalAptitudeTwo.aptitudePicture)
                    console.log(data.amPersonalAptitudeThree.aptitudePicture)
        ...
        $("button[name='reset']").on("click", function () {
            location.href = "<%=path%>/queryCompareResult/querySameCompareResultList";
        })
