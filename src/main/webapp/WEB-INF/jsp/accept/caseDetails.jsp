                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <p style="color: red;display:none;" id="jianding">*鉴定要求不能为空</p>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    <%--&lt;%&ndash;try {&ndash;%&gt;--%>
    <%--&lt;%&ndash;var ws = new WebSocket("ws://localhost:8171/");&ndash;%&gt;--%>
    <%--&lt;%&ndash;// when data is comming from the server, this metod is called&ndash;%&gt;--%>
    <%--&lt;%&ndash;ws.onmessage = function (evt) {&ndash;%&gt;--%>
    <%--&lt;%&ndash;if(evt.data != "0"){&ndash;%&gt;--%>
    <%--&lt;%&ndash;$("#signPic").attr("src", "data:image/png;base64," + evt.data);&ndash;%&gt;--%>
    <%--&lt;%&ndash;ws.close();&ndash;%&gt;--%>
    <%--&lt;%&ndash;}&ndash;%&gt;--%>
    <%--&lt;%&ndash;};&ndash;%&gt;--%>
    <%--&lt;%&ndash;// when the connection is established, this method is called&ndash;%&gt;--%>
    <%--&lt;%&ndash;ws.onopen = function () {&ndash;%&gt;--%>
    <%--&lt;%&ndash;$("#deviceStatus").val(".. connection open");&ndash;%&gt;--%>
    <%--&lt;%&ndash;setInterval(function(){&ndash;%&gt;--%>
    <%--&lt;%&ndash;ws.send("1");&ndash;%&gt;--%>
    <%--&lt;%&ndash;},1000);&ndash;%&gt;--%>
    <%--&lt;%&ndash;};&ndash;%&gt;--%>
    <%--&lt;%&ndash;// when the connection is closed, this method is called&ndash;%&gt;--%>
    <%--&lt;%&ndash;ws.onclose = function () {&ndash;%&gt;--%>
    <%--&lt;%&ndash;$("#deviceStatus").val(".. connection closed");&ndash;%&gt;--%>
    <%--&lt;%&ndash;alert("1111")&ndash;%&gt;--%>
    <%--&lt;%&ndash;if (window.confirm("你可能还没有安装签名程序，请下载安装")) {&ndash;%&gt;--%>
    <%--&lt;%&ndash;alert("5555")&ndash;%&gt;--%>
    <%--&lt;%&ndash;location.href="<%=path%>/center/exportDocument";&ndash;%&gt;--%>
    ...
    <%--&lt;%&ndash;} catch (e) {&ndash;%&gt;--%>
    <%--&lt;%&ndash;alert(e)&ndash;%&gt;--%>
    <%--&lt;%&ndash;//没有程序路径 提示下载&ndash;%&gt;--%>
    <%--&lt;%&ndash;if (window.confirm("你可能还没有安装签名程序，请下载安装")) {&ndash;%&gt;--%>
    <%--&lt;%&ndash;alert("1222")&ndash;%&gt;--%>
    <%--&lt;%&ndash;location.href="<%=path%>/center/exportDocument";&ndash;%&gt;--%>
    <%--&lt;%&ndash;return false;&ndash;%&gt;--%>
    <%--&lt;%&ndash;} else {&ndash;%&gt;--%>
    <%--&lt;%&ndash;return false;&ndash;%&gt;--%>
    ...
    <%--// var inc = document.getElementById('incomming');//提示信息--%>
    <%--var wsImpl = window.WebSocket || window.MozWebSocket;--%>

    <%--// inc.innerHTML += "connecting to server ..<br/>";//提示信息链接服务--%>

    <%--window.ws = new wsImpl('ws://localhost:8171/');//创建websocket通信--%>

    <%--ws.onmessage = function (evt) {//获取回传信息--%>
    <%--wsUupload = 1;--%>

    <%--// inc.innerHTML += evt.data + '<br/>';//提示回传信息--%>
    <%--if (evt.data != 0) {//判断回传值为空0无图片--%>

    <%--if (evt.data != 1) {--%>
    <%--clearInterval(timer)//结束循环传值--%>
    <%--var signPicImage = $("#signPic").attr("src", "data:image/png;base64," + evt.data);--%>
    <%--$("#signPices").attr("src", "data:image/png;base64," + evt.data);--%>
    <%--clearInterval(timer)//结束循环传值--%>
    <%--ws.close();--%>

    <%--} else {--%>

    <%--clearInterval(closedialog)//结束循环传值--%>
    <%--$(".shadow").hide();--%>
    ...
    <%--ws.onopen = function () {//打开通信--%>

    <%--timer = setInterval(function () {--%>
    <%--ws.send("1");--%>
    <%--}, 2000);//循环传值--%>
    <%--closedialog = setInterval(function () {--%>
    <%--$(".shadow").show();--%>
    <%--}, 1000)--%>

    <%--// inc.innerHTML += '.. connection open<br/>';//提示打开通信--%>
    <%--};--%>

    <%--ws.onclose = function () {//结束通信--%>
    <%--// inc.innerHTML += '.. connection closed<br/>';//提示结束通信--%>
    <%--clearInterval(timer)//结束循环传值--%>
    <%--if (wsUupload == 0) {--%>
    <%--if (window.confirm("你可能还没有安装签名程序，请下载安装")) {//提示下载--%>
    <%--location.href = "<%=path%>/center/exportDocument";--%>
    <%--} else {--%>
        ...
        $('.addMaterialEvidencerlSample').click(function () {
            ...
            if (form.data('bootstrapValidator').isValid()) {
                ...
                if (index != "") {
                    ...
                    newTr += '<input type="hidden" name="inspectionObjective" value="' + inspectionObjective + '">'
                    newTr += '</td>'
                    newTr += '</tr>'
