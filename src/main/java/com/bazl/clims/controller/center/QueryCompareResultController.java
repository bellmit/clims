...
import com.bazl.clims.model.vo.MatchRelativeResultVo;
import com.bazl.clims.model.vo.MatchSameResultVo;
import com.bazl.clims.service.*;
import com.bazl.clims.utils.*;
import com.bazl.clims.webservice.basicInfo.ArrayOfString;
import com.bazl.clims.webservice.basicInfo.ArrayOfWSPanel;
import com.bazl.clims.webservice.basicInfo.BasicInfoPortType;
import com.bazl.clims.webservice.basicInfo.WSPanel;
import com.bazl.clims.webservice.caseSampleLib.CaseSampleLibPortType;
import com.bazl.clims.webservice.caseSampleLib.SampleNoDomain;
import com.bazl.clims.webservice.domain.CaseParentageMatchResult;
import com.bazl.clims.webservice.domain.LibMatchResultGroupDomain;
import com.bazl.clims.webservice.domain.TotalRelativeMatchResult;
import com.bazl.clims.webservice.services.CaseInfo;
import com.bazl.clims.webservice.services.QueryDataServicePortType;
import com.bazl.clims.webservice.util.MarkerAlleleUtil;
...
public class QueryCompareResultController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("/querySameCompareResultList")
    public ModelAndView querySameCompareResultList(HttpServletRequest request, MatchSameResultVo matchResult, PageInfo pageInfo) {
        ...
        try {
            ...
            List<DictItem> dictItemList = dictItemService.selectListByDictTypeCode(Constants.COMPARISON_CATEGORY);
            List<String> categoryList = new ArrayList<>();

            if (StringUtils.isNotBlank(matchResult.getComparisonCategory())) {
                matchResult2.setComparisonCategory(matchResult.getComparisonCategory().trim());
            } else if (StringUtils.isNotBlank(matchResult.getComparisonCategory()) && matchResult.getComparisonCategory().equals("05")) {
                for (DictItem category : dictItemList) {
                    if (category.getDictCode().equals("02") || category.getDictCode().equals("03") || category.getDictCode().equals("04")) {
                        categoryList.add(category.getDictCode());
                    }
                }
                matchResult2.setCategoryList(categoryList);
                matchResult.setComparisonCategory("05");
            } else {
                for (DictItem category : dictItemList) {
                    if (category.getDictCode().equals("02") || category.getDictCode().equals("03") || category.getDictCode().equals("04")) {
            ...
            if ("0".equals(limsEdition)) {
                matchResult2.setAcceptOrgId(userOrgId);
            }

            //查询所有组id
            List<MatchSameResultVo> matchCaseResultList = matchSameResultService.selectStrandGroupAll(matchResult2, pageInfo);
            int totalCnt = matchSameResultService.selectStrandVOCount(matchResult2);
            if (matchCaseResultList != null) {
                for (MatchSameResultVo matchSameResultVo : matchCaseResultList) {
                    ...
                    /*if (StringUtils.isNotBlank(matchResult.getComparisonCategory())) {
                        ...
                        if (StringUtils.isNotBlank(matchResult.getComparisonCategory())) {
                            ...
                            matchResult1.setCategoryList(categoryList);
                            matchResult.setComparisonCategory("05");
                        } else {
                            for (DictItem category : dictItemList) {
                                if (category.getDictCode().equals("02") || category.getDictCode().equals("03") || category.getDictCode().equals("04")) {
                        ...
                        if ("0".equals(limsEdition)) {
                            matchResult1.setAcceptOrgId(userOrgId);
                        }
                        //根据组id查询比对结果
                        ...
                        List<MatchSameResultVo> collect1 = collect.stream().filter(distinctByKey(MatchSameResultVo::getCaseId)).collect(Collectors.toList());
                        ...
                        //串并案同一分组里若既有人员又有物证,只显示比中的人员样本
                        List<MatchSameResultVo> resultVoList = new ArrayList<>();
                        for (int i = 0; i < collect.size(); i++) {
                            MatchSameResultVo msrVo1 = collect.get(i);
                            if (msrVo1 != null) {
                                boolean flag = true;
                                for (int j = i + 1; j < collect.size(); j++) {
                                    MatchSameResultVo msrVo2 = collect.get(j);
                                    if (msrVo1 != null && msrVo2 != null) {
                                        //判断是否是同一个案件下
                                        if (StringUtils.isNotBlank(msrVo1.getCaseId()) && StringUtils.isNotBlank(msrVo2.getCaseId())) {
                                           if (msrVo1.getCaseId().equals(msrVo2.getCaseId())) {
                                               //如果既有人员检材又有物证检材，只保留人员检材
                                               if (Constants.SAMPLE_FLAG_0.equals(msrVo1.getSampleFlag()) &&
                                                       Constants.SAMPLE_FLAG_1.equals(msrVo2.getSampleFlag())) {
                                                   //msrVo1中检材为物证，msrVo2为人员，故flag变成false，去除此检材
                                                   flag = false;
                                               }
                                           }
                                        }
                                ...
                                if (flag) {
                                    resultVoList.add(msrVo1);
                                }
                            }
                        }
                        result2.put(matchSameResultVo.getEntity().getGroupId(), resultVoList);
                        result.put(matchSameResultVo.getEntity().getGroupId(), collect1);
                    }
                }
            }
            view.addObject("orgInfoList", orgInfoList);
            ...
            view.addObject("matchResultList", result2);
            view.addObject("resultList", result);
            view.addObject("result2List", result2);
     ...
     * @return
     */
    @RequestMapping("/queryAreaSameCompareResultList")
    public ModelAndView queryAreaSameCompareResultList(HttpServletRequest request, MatchSameResultVo matchResult, PageInfo pageInfo) {
        ModelAndView view = new ModelAndView();
        ...
        try {
            ...
            if (StringUtils.isNotBlank(userOrgId) && userOrgId.contains("110023")) {
                ...
                if(null != matchResult.getAcceptOrgId()){
                    userOrgId = matchResult.getAcceptOrgId();
                }
            }
            if(null != matchResult.getAcceptOrgId() && StringUtils.isBlank(loaUserInfo.getOrgId())){
                userOrgId = matchResult.getAcceptOrgId();
            }
           /* matchResult.setUserOrdId(userOrgId);
            if(StringUtils.isNotBlank(userOrgId)){
                ...
                for (int i = 0; i < matchResultGroupList.size(); i++) {
                    ...
                    if (libMatchResultGroupDomain != null) {
                        ...
                        for (Map<String, String> samplemap : sampleList) {
                            ...
                            /*if(!location.equals(WordFactory.LOCATION_BEIJING)){
                                DnaSample dnaSample = submitToNationalDNALibService.queryDnaSample(samplemap.get("sampleid").toString());
                                if(dnaSample!=null){
                                    if(dnaSample.getLevelflag()==1)
                                        levelflag++;
                                }
                            }*/
                        }

                       /* if (!location.equals("BEIJING"))
                        {
                            libMatchResultGroupDomain.setLevelflag(levelflag);
                        }*/
            ...
            for (int i = 0; i < matchResultGroupList.size(); i++) {
                ...
                if (libMatchResultGroupDomain != null) {
                    ...
                    for (Map<String, String> samplemap : sampleList) {
                        ...
                        if (null != sampleName) {
                            if (sampleName.equals(samplemap.get("samplename"))) {
                                flag = "1";
                            }else{
                                flag = "0";
                                break;
                            ...
                            sampleName = samplemap.get("samplename");
                        }
                        if (samplemap.get("sampletype").equals("3") || samplemap.get("sampletype").equals("4") || samplemap.get("sampletype").equals("5") || samplemap.get("sampletype").toString().equals("6") || samplemap.get("sampletype").equals("8") || samplemap.get("sampletype").equals("9") || samplemap.get("sampletype").equals("10")) {
                            personCnt++;
                        }
                    }
                    ...
                    for (Map<String, String> samplemap : sampleList) {
                        if (samplemap.get("sampletype").equals("3") || samplemap.get("sampletype").equals("4") || samplemap.get("sampletype").equals("5") || samplemap.get("sampletype").toString().equals("6") || samplemap.get("sampletype").equals("8") || samplemap.get("sampletype").equals("9") || samplemap.get("sampletype").equals("10")) {
                            samplemap.put("flag", flag);
                            samplemap.put("personCnt", String.valueOf(personCnt));
                        }
                    }
                }
            }
            ...
            view.addObject("pageInfo", pageInfo.updatePageInfo(recordCount));
            view.addObject("comparisonCategory", matchResult.getComparisonCategory());
            view.addObject("matchResult", matchResult);
            view.setViewName("query/areaSameCompareResultList");
        } catch (Exception e) {
     ...
     * @return
     */
    @RequestMapping("/queryAreaSameCompareResultDetail")
    public ModelAndView queryAreaSameCompareResultDetail(MatchSameResultVo matchResult, PageInfo pageInfo) {
        ...
        try {
            ...
            view.addObject("matchResult", matchResult);
            view.addObject("pageInfo", pageInfo.updatePageInfo(recordCount));
            view.setViewName("query/viewAreaSameCompareDetail");
        } catch (Exception e) {
     ...
     * @return
     */
    @RequestMapping("/queryAreaSameCompareCondition")
    ...
    public ModelAndView queryAreaRelationCompareResultList(HttpServletRequest request, MatchRelativeResultVo matchResult, PageInfo pageInfo,
        ...
        try {
            ...
            if(null != matchResult.getAcceptOrgId() && StringUtils.isBlank(operateUser.getOrgId())){
                userOrgId = matchResult.getAcceptOrgId();
            }
            ...
            view.addObject("comparisonCategory", matchResult.getComparisonCategory());
            view.addObject("matchResult", matchResult);
            view.setViewName("query/areaRelationCompareResultList");
        } catch (Exception e) {
     ...
     * @return
     */
    @RequestMapping("/queryAreaRelationCompareResultDetail")
    public ModelAndView queryAreaRelationCompareResultDetail(MatchRelativeResultVo matchResult, PageInfo pageInfo) {
        ...
        try {
            ...
            view.addObject("matchResult", matchResult);
            view.addObject("pageInfo", pageInfo.updatePageInfo(recordCount));
            view.setViewName("query/viewAreaRelationCompareDetail");
        } catch (Exception e) {
     ...
     * @return
     */
    @RequestMapping("/queryAreaRelationCompareCondition")
     ...
     * @return
     */
    @RequestMapping("/exportQyListExcel")
    public ModelAndView exportQyListExcel(HttpServletRequest request, HttpServletResponse response, MatchRelativeResultVo matchResult, PageInfo pageInfo) {
    ...
    private List<Map<String, Object>> createQyExcelRecord(List<MatchRelativeResultVo> list) {
        ...
        for (int j = 0; j < list.size(); j++) {
            ...
            if (null != vote.getInstoredaType()) {
                switch (Integer.parseInt(vote.getInstoredaType())) {
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        mapValue.put("2", "嫌疑人亲属");
                        break;
                    case 9:
                        mapValue.put("2", "受害人亲属");
                        break;
                    case 10:
                        mapValue.put("2", "失踪人口亲属");
                        break;
                    case 11:
                        mapValue.put("2", "基础库");
                        break;
            ...
            if (null != vote.getInstoredaType()) {
                switch (Integer.parseInt(vote.getInstoredaType())) {
                    case 1:
                        mapValueb.put("2", "现场物证");
                        break;
                    case 2:
                        mapValueb.put("2", "混合物证");
                        break;
                    case 3:
                        mapValueb.put("2", "违法犯罪人员");
                        break;
                    case 4:
                        mapValueb.put("2", "嫌疑人");
                        break;
                    case 5:
                        ...
                        break;
                    case 6:
                        mapValueb.put("2", "失踪人口");
                        break;
                    case 7:
                        mapValueb.put("2", "无名尸");
                        break;
                    case 8:
                        mapValueb.put("2", "嫌疑人亲属");
                        break;
                    case 9:
                        mapValueb.put("2", "受害人亲属");
                        break;
                    case 10:
                        mapValueb.put("2", "失踪人口亲属");
                        break;
                    case 11:
                        mapValueb.put("2", "基础库");
                        break;
                    case 12:
                        mapValueb.put("2", "YSTR");
                        break;
                    default:
                        mapValueb.put("2", "");
                        break;
            ...
            if (null != vote.getInstoredaType()) {
                switch (Integer.parseInt(vote.getInstoredaType())) {
                    case 1:
                        mapValuec.put("2", "现场物证");
                        break;
                    case 2:
                        mapValuec.put("2", "混合物证");
                        break;
                    case 3:
                        mapValuec.put("2", "违法犯罪人员");
                        break;
                    case 4:
                        mapValuec.put("2", "嫌疑人");
                        break;
                    case 5:
                        ...
                        break;
                    case 6:
                        mapValuec.put("2", "失踪人口");
                        break;
                    case 7:
                        mapValuec.put("2", "无名尸");
                        break;
                    case 8:
                        mapValuec.put("2", "嫌疑人亲属");
                        break;
                    case 9:
                        mapValuec.put("2", "受害人亲属");
                        break;
                    case 10:
                        mapValuec.put("2", "失踪人口亲属");
                        break;
                    case 11:
                        mapValuec.put("2", "基础库");
                        break;
                    case 12:
                        mapValuec.put("2", "YSTR");
                        break;
                    default:
                        mapValuec.put("2", "");
                        break;
     ...
     * @return
     */
    @RequestMapping("/queryRelativeCompareResultList")
    public ModelAndView queryRelativeCompareResultList(HttpServletRequest request, MatchRelativeResultVo matchResult, PageInfo pageInfo,
        ...
        try {
            ...
            /*if("0".equals(limsEdition)){
                matchRelativeResultVo.setDelegateOrgCode("110105000000");
            }*/
            //查询比对结果
            List<MatchRelativeResultVo> matchRelativeResultList = matchRelativeResultService.selectMatchRelativeResultList(matchRelativeResultVo, pageInfo);
...
*/
     ...
     * @return
     */
    @RequestMapping("/queryCompareResultDetail")
    public ModelAndView queryCompareResultDetail(HttpServletRequest request, String groupId, MatchSameResult matchSameResult, PageInfo pageInfo, String groupNo,String sampleNo) {
        ModelAndView view = new ModelAndView();
        //获取当前登录人信息
        Subject subject = SecurityUtils.getSubject();
        LoaUserInfo operateUser = (LoaUserInfo) subject.getPrincipal();
        ...
        try {
            ...
            if (StringUtils.isNotBlank(sampleNo)) {
                matchSameResult.setSampleaNo(sampleNo.trim());
                matchSameResult.setSamplebNo(sampleNo.trim());
            }
            if (StringUtils.isNotBlank(userOrgId)){
                matchSameResult.setAcceptOrgId(userOrgId);
            }
            List<MatchSameResult> matchResultList2 = new ArrayList<>();
            ...
            if (null != matchResultList && matchResultList.size() > 0) {
                ...
                for (MatchSameResult matchSameResult1 : matchResultList) {
                    if (matchSameResult1.getSampleaNo().equals(sampleNo) || matchSameResult1.getSamplebNo().equals(sampleNo)){
                        matchResultList2.add(matchSameResult1);
                    }
...
//            view.addObject("matchResultList", matchResultList);
            view.addObject("matchResultList", matchResultList2);
     ...
     * @return
     */
    @RequestMapping("/queryRelativeCompareResultDetail")
    public ModelAndView queryRelativeCompareResultDetail(HttpServletRequest request, String groupId, MatchRelativeResult matchRelativeResult, String id, String groupNo) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(userOrgId)){
                matchRelativeResult.setAcceptOrgId(userOrgId);
            }
    ...
    public void chuanbindan(HttpServletRequest request, HttpServletResponse response, String groupId) {
        ...
        for (MatchSameResult matchSameResult : matchSameResultList) {
            List<LimsSampleInfoDna> limsSampleInfoDnas = limsSampleInfoDnaService.selectBySampleNo(matchSameResult.getSampleaNo());

            for(LimsSampleInfoDna sampleInfo:limsSampleInfoDnas){
                if (StringUtils.isNotBlank(sampleInfo.getCoreVictimStats()) ){
                    if (sampleInfo.getCoreVictimStats().equals("1")){
                        matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(事主)");
                    }
//                    if(sampleInfo.getCoreVictimStats().equals("0")){
//                        matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(非事主)");
//                    }
                }

                if (StringUtils.isNotBlank(sampleInfo.getCoreTakenStats()) ){
                    if (sampleInfo.getCoreTakenStats().equals("1")){
                        matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(中心提取)");
                    }
//                    if(sampleInfo.getCoreTakenStats().equals("0")){
//                        matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(非中心提取)");
//                    }
                }

            }
           /* if (StringUtils.isNotBlank(limsSampleInfoDnas.get(0).getCoreTakenStats())){
                if (limsSampleInfoDnas.get(0).getCoreTakenStats().equals(0)){
                    matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(非中心提取)");
                }else{
                    matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(中心提取)");
                }
            }
            if (StringUtils.isNotBlank(limsSampleInfoDnas.get(0).getCoreVictimStats())){
                if (limsSampleInfoDnas.get(0).getCoreVictimStats().equals(1)){
                    matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(事主)");
                }else{
                    matchSameResult.setSampleaNo(matchSameResult.getSampleaNo()+"(非事主)");
                }
     ...
     * @param response
     */
    @RequestMapping("/chuanbindanrelative")
        ...
        if (ListUtils.isNotNullAndEmptyList(matchRelativeResultList)) {
            for (MatchRelativeResult mrr : matchRelativeResultList) {
                ...
                if (sampleInfoDnaA != null) {
                    ...
                    List<LimsSampleInfoDna> limsSampleInfoDnas = limsSampleInfoDnaService.selectBySampleNo(mrr.getSampleaNo());

                    for(LimsSampleInfoDna sampleInfo:limsSampleInfoDnas){
                        if (StringUtils.isNotBlank(sampleInfo.getCoreVictimStats()) ){
                            if (sampleInfo.getCoreVictimStats().equals("1")){
                                mrr.setSampleaNo(mrr.getSampleaNo()+"(事主)");
                            }
//                            if(sampleInfo.getCoreVictimStats().equals("0")){
//                                mrr.setSampleaNo(mrr.getSampleaNo()+"(非事主)");
//                            }
                        }

                        if (StringUtils.isNotBlank(sampleInfo.getCoreTakenStats()) ){
                            if (sampleInfo.getCoreTakenStats().equals("1")){
                                mrr.setSampleaNo(mrr.getSampleaNo()+"(中心提取)");
                            }
//                            if(sampleInfo.getCoreTakenStats().equals("0")){
//                                mrr.setSampleaNo(mrr.getSampleaNo()+"(非中心提取)");
//                            }
                        }
     ...
     * @return
     */
    @RequestMapping("/queryCompareCondition")
    public ModelAndView queryCompareCondition(HttpServletRequest request, String id) {
        ...
        try {
            ...
            if(StringUtils.isNotBlank(matchSameResult.getCaseaId())){
                List<LimsConsignmentInfo> limsConsignmentInfoList = limsConsignmentInfoService.selectByCaseId(matchSameResult.getCaseaId());
                if (limsConsignmentInfoList.size()>0){
                    for(LimsConsignmentInfo consignmentInfo:limsConsignmentInfoList){
                        if (StringUtils.isNotBlank(consignmentInfo.getAcceptorId()) &&  consignmentInfo.getAppendFlag().equals("0")){
                            AmPersonalInfo amPersonalInfo = amPersonalInfoService.selectByPersonalId(consignmentInfo.getAcceptorId());
                            if (null != amPersonalInfo && StringUtils.isNotBlank(amPersonalInfo.getFullName())){
                                matchSameResult.setAcceptAName(amPersonalInfo.getFullName());
                            }
                        }
                    }
                }
            }
            if(StringUtils.isNotBlank(matchSameResult.getCasebId())){
                List<LimsConsignmentInfo> limsConsignmentInfoList = limsConsignmentInfoService.selectByCaseId(matchSameResult.getCasebId());
                if (limsConsignmentInfoList.size()>0){
                    for(LimsConsignmentInfo consignmentInfo:limsConsignmentInfoList){
                        if (StringUtils.isNotBlank(consignmentInfo.getAcceptorId()) &&  consignmentInfo.getAppendFlag().equals("0")){
                            AmPersonalInfo amPersonalInfo = amPersonalInfoService.selectByPersonalId(consignmentInfo.getAcceptorId());
                            if (null != amPersonalInfo && StringUtils.isNotBlank(amPersonalInfo.getFullName())){
                                matchSameResult.setAcceptBName(amPersonalInfo.getFullName());
                            }
                        }
                    }
                }
            ...
            if (StringUtils.isNotBlank(matchSameResult.getUpdatePerson())){
                LoaUserInfo loaUserInfo = loaUserInfoService.selectByUserName2(matchSameResult.getUpdatePerson());
                if (null != loaUserInfo && StringUtils.isNotBlank(loaUserInfo.getAmPersonalInfo().getFullName())){
                    matchSameResult.setUpdatePerson(loaUserInfo.getAmPersonalInfo().getFullName().trim());
                }else{
                    matchSameResult.setUpdatePerson(null);
                }
            }
     ...
     * @return
     */
    @RequestMapping("/queryRelativeCompareCondition")
    public ModelAndView queryRelativeCompareCondition(HttpServletRequest request, String id) {
        ModelAndView view = new ModelAndView();
        try {
            //根据id查询亲缘比对结果
            ...
            if (StringUtils.isNotBlank(matchRelativeResult.getCaseaId())){
                List<LimsConsignmentInfo> limsConsignmentInfoList = limsConsignmentInfoService.selectByCaseId(matchRelativeResult.getCaseaId());
                for (LimsConsignmentInfo consignmentInfo:limsConsignmentInfoList){
                    if(consignmentInfo.getAppendFlag().equals("0") && StringUtils.isNotBlank(consignmentInfo.getAcceptorId())){
                        AmPersonalInfo amPersonalInfo = amPersonalInfoService.selectByPersonalId(consignmentInfo.getAcceptorId());
                        if (null != amPersonalInfo && StringUtils.isNotBlank(amPersonalInfo.getFullName())){
                            matchRelativeResult.setAcceptAName(amPersonalInfo.getFullName());
                        }
                    }
                }
            }

            if (StringUtils.isNotBlank(matchRelativeResult.getCasebId())){
                List<LimsConsignmentInfo> limsConsignmentInfoList = limsConsignmentInfoService.selectByCaseId(matchRelativeResult.getCasebId());
                for (LimsConsignmentInfo consignmentInfo:limsConsignmentInfoList){
                    if(consignmentInfo.getAppendFlag().equals("0") && StringUtils.isNotBlank(consignmentInfo.getAcceptorId())){
                        AmPersonalInfo amPersonalInfo = amPersonalInfoService.selectByPersonalId(consignmentInfo.getAcceptorId());
                        if (null != amPersonalInfo && StringUtils.isNotBlank(amPersonalInfo.getFullName())){
                            matchRelativeResult.setAcceptBName(amPersonalInfo.getFullName());
                        }
                    }
                }
            }
     ...
     * @return
     */
    @RequestMapping("/relieveGroupsRelevance")
    @ResponseBody
     ...
     * @return
     */
    @RequestMapping("/relieveRelevance")
    @ResponseBody
     ...
     * @return
     */
    @RequestMapping("/relieveRelativeRelevance")
    @ResponseBody
     ...
     * @return
     */
    @RequestMapping("/relieveRelativeGroupsRelevance")
    @ResponseBody
     ...
     * @return
     */
    @RequestMapping("/confirmCompareGroup")
    @ResponseBody
     ...
     * @return
     */
    @RequestMapping("/confirmRelativeCompareGroup")
    @ResponseBody
     ...
     * @return
     */
    @RequestMapping("/confirmRelativeCompare")
    @ResponseBody
